...
// 初回認証時処理
// Userドキュメントを作成する
export const createUser = defaultFunctions.auth.user().onCreate(async (user: admin.auth.UserRecord) => {
  let result = 'error';
...
export const deleteUser = defaultFunctions.auth.user().onDelete(async (user: admin.auth.UserRecord) => {
  let result = 'error';
  let message = 'function failed';
  const mainFunc = async (user: admin.auth.UserRecord) => {
    // 各種コレクション削除
    await deleteCollection(db, db.collection('users').doc(user.uid).collection('task_orders').path, 500);
